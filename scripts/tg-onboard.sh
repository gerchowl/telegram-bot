...
main() {
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      -d | --dir)
        ...
        ;;
      -h | --help)
        echo "usage: tg-onboard [--dir PROJECT_DIR]"
        return 0
        ;;
      *)
  ...
  token="$(step_token)"
  local username
  username="$(step_validate "$token")"
  step_sops "$token"
  local chat_id
  chat_id="$(step_chat_id "$token" "$username")"
...
gen_age_key() {
  ...
  [ -e "$loc" ] && {
    err "refusing to overwrite existing file: $loc"
    exit 1
  }
  mkdir -p "$(dirname "$loc")"
  age-keygen -o "$loc" >/dev/null 2>&1 || {
    err "age-keygen failed"
    exit 1
  }
  chmod 600 "$loc"
...
step_sops() {
  ...
  if [ "${#keys[@]}" -gt 0 ]; then
    ...
    case "$ans" in
      [pP]*)
        read -r -p "   Path to existing age key file: " key_file
  ...
  { [ -n "$key_file" ] && [ -f "$key_file" ]; } || {
    err "no usable age key file: ${key_file:-<none>}"
    exit 1
  }
  grep -q 'AGE-SECRET-KEY-' "$key_file" 2>/dev/null || {
    err "$key_file is not an age private key"
    exit 1
  }
  recipient="$(age-keygen -y "$key_file" 2>/dev/null | head -n1)"
  [ -n "$recipient" ] || {
    err "could not derive a recipient from $key_file"
    exit 1
  }
...
step_config() {
  ...
  local abs_sops
  abs_sops="$(pwd)/secrets/telegram.yaml"
